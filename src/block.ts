import { BlockObject, BlockObjectType,
         OutpointObjectType, TransactionObject, ObjectType } from './message'
  ...
  async validateTx(peer: Peer, stateBefore: UTXOSet, height: number) {
    ...
    for (const maybeTx of maybeTransactions) {
      ...
      for (const input of tx.inputs) {
        if (outpoints.includes(input.outpoint)) {
          throw new Error(`Block reports a transaction with multiple inputs going to the same outpoint`)
        }
    ...
    // check miner and note fields for ascii 
    if (this.miner !== undefined && this.miner.length > 128) {
      throw new Error(`Miner field has more than 128 characters. Rejecting Block.`)
    }
    if (this.note !== undefined && this.note.length > 128) {
      throw new Error(`Note field has more than 128 characters. Rejecting Block.`)
    }
    if (this.note !== undefined && !/^[\u0000-\u007f]*$/.test(this.note)) {
      throw new Error(`Note field has more non ascii characters. Rejecting Block.`)
    }
    if (this.miner !== undefined && !/^[\u0000-\u007f]*$/.test(this.miner)) {
      throw new Error(`Note field has more non ascii characters. Rejecting Block.`)
    }
